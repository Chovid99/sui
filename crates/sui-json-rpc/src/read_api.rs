...
impl ReadApi {
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        let timestamps = self
            ...
            .tap_err(|e| {
                error!("Failed to fetch checkpoint summaries by these checkpoint ids: {unique_checkpoint_numbers:?} with error: {e:?}");
            })?
            .into_iter()
        ...
        if opts.show_events {
            ...
            let events = self
                ...
                .tap_err(|e| {
                    error!("Failed to call multi_get_events for transactions {digests:?} with event digests {event_digests_list:?}: {e:?}");
                })?
                .into_iter();
    ...
    async fn get_object(
        ...
        with_tracing!("get_object", async move {
            ...
            let object_read = spawn_monitored_task!(async move {
                state.get_object_read(&object_id).tap_err(|e| {
